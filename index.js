...
function isFail(row,col){ //是否失败
	var rCol = col-0+1,lCol = col-1;
	var downRow = row-0+1,upRow = row-1;
	if(($$(row + '_' + rCol).className == 'wall'&&($$(downRow + '_' + col).className == 'wall'||$$(upRow + '_' + rCol).className == 'wall'))||($$(row + '_' + lCol).className == 'wall'&&($$(downRow + '_' + col).className == 'wall'||$$(upRow + '_' + rCol).className == 'wall'))){
		alert('您已失败，请重试!');
		initMap(gameData[parseInt($('select').value.substring(1, 3)) - 1]);
	}
}
